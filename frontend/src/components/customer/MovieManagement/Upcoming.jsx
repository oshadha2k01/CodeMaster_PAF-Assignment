...
//           <p className="text-silver text-lg">Get ready for the next big releases</p>
//         </div>
...
import MainNavBar from '../../navbar/MainNavBar';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faSearch, faFilter, faClock, faTicketAlt, faPlay, faTimes, faCalendarAlt } from '@fortawesome/free-solid-svg-icons';
import { ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
  ...
  const fetchMovies = async () => {
    try {
      ...
      if (data.success && Array.isArray(data.data)) {
        ...
        // Original filtering: 
        // const upcomingMovies = data.data.filter(movie => movie.status === 'Upcoming');
        console.log('Upcoming Movies:', upcomingMovies); // Debug: Check filtered movies
        setMovies(upcomingMovies);
        setError(null);
      } else {
        throw new Error('Invalid data format received from server');
    ...
    <div className="min-h-screen bg-deep-space pt-16">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            <p className="text-silver text-lg">No upcoming movies found.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filteredMovies.map((movie) => (
